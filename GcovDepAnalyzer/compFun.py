...
def findFun(dict, fileName, lines, obj, missFileList):
    ...
    for item in dict[fileName]:
        ...
        # print "filename:", fileName, "obj:", obj
        for i in range(start, end):
            line = lines[i].strip()
            if line.endswith(obj):
                if line.split(',')[0].split(':')[1] != '0':
                    # print line, i
                    return True
                else:
                    return False
...
for i in range(0, len(lineList1)):
    ...
    if line.strip() == "TN:":
        ...
        if line.split(',')[0].split(':')[1] != '0':
            # print "in!"
            if findFun(dictFile2, fileName1, lineList2, funName, missFileList) == True:
                continue
            else:
                if (fileName1, funName) not in missLineList:
                    missLineList.append((fileName1, funName))
                continue
